...
if(isset($_POST['b']))
{
		$eid=$_SESSION['sessionVar20'];
		 $user_code=$_POST['user_code']; 
		 $pw1=$_POST['pw1']; 
		 $pw2=$_POST['pw2'];
		 $result = mysql_query("select system_code from password WHERE id='$eid'");
		 $row=mysql_fetch_array($result);
		 $system_code=$row[0]; 
		 $query1=mysql_query("update password set user_code='$user_code' where id='$eid'");
		  if($user_code==$system_code)
		   {
			  $query=mysql_query("update employees set password='$pw2' where id='$eid'");
			   if($query)
			   {
